...
<Select.Root
	...
	bind:open
>
	<slot name="trigger" text={get(currentTranslations)[`Dropdown item ${stringValue}`] || stringValue} {open}>
		<Select.Trigger
		...
		>
			<div class="text-slate-300 text-left flex-grow flex-shrink truncate text-{size}">
				{get(currentTranslations)[`Dropdown item ${stringValue}`] || stringValue}
			</div>
			<Icon
	...
	>
		{#each items as item}
			<slot name="item" {item}>
				...
				>
					{get(currentTranslations)[`Dropdown item ${getLabel(item)}`] || getLabel(item)}
