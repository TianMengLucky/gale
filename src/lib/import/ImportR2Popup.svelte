...
<Popup bind:open title="{get(currentTranslations)['Import from R2']}" canClose={!loading} maxWidth="[55%]">
	<div class="text-slate-300">
		<p>{@html get(currentTranslations)['Import from R2 description 1']}</p>

		<p class="mt-2">{@html get(currentTranslations)['Import from R2 description 2']}</p>

		<p class="mt-2">{@html get(currentTranslations)['Import from R2 description 3']}</p>
	</div>
	...
	<div class="flex gap-2 justify-end w-full mr-0.5 mt-3">
		{#if importData?.r2modman || importData?.thunderstore}
			<BigButton color="gray" on:click={() => (open = false)}>{get(currentTranslations)["Cancel"]}</BigButton>
			<BigButton color="green" on:click={doImport}>{get(currentTranslations)["Import"]}</BigButton>
		{/if}
	</div>
</Popup>
